...
def get_afk_users_id_after_date(guild_id: int, daysToCheck: int, guildMembersIdList) -> tuple:
    ...
    active_users_id_list = Repos.get_active_users_id_after_date(guild_id=guild_id, date=date)
    return tuple(filter(lambda val: val not in active_users_id_list, guildMembersIdList))
